...
1\. \*\*`Vision\_Controlled\_Object\_Tracking.py`\*\*: Script de Python que utiliza OpenCV para detectar un objeto por color (HSV), calcula su centro de masa y envía las coordenadas mapeadas vía serial.

2\. \*\*`Vision\_Controlled\_Eye\_Animation.ino`\*\*: Código para el ESP32 que recibe las coordenadas, gestiona el borrado selectivo de pixeles para evitar parpadeos y ejecuta una animación de parpadeo aleatorio.
...
3\. En el script `Vision\_Controlled\_Object\_Tracking.py`, asegúrate de que la variable `SERIAL\_PORT` apunte al puerto correcto (ej. `/dev/ttyUSB0` en Linux o `COMX` en Windows).
...
&nbsp;  python Vision\_Controlled\_Object\_Tracking.py
